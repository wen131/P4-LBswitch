...
with open("time_ecmp_f",'r') as f:
  time_ecmp=numpy.array(list(map(lambda x: float(x)*5,f.read().split("\n")[:-1])))
with open("time_weight_f",'r') as f:
  time_weig=numpy.array(list(map(lambda x: float(x)*5,f.read().split("\n")[:-1])))
with open("time_static_f",'r') as f:
...
plt.ylabel("CDF (%)")
plt.tight_layout()
plt.savefig('heavy_no.eps', format='eps' ,dpi=1000)
